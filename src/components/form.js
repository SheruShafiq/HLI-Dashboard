...
function App() {
    ...
    onSuccess: async (response) => {
      try {
        ...
        const res = await axios.get(
          `https://www.googleapis.com/analytics/v3/data/realtime?ids=ga:${gaID}&metrics=rt:activeUsers&access_token=${response.access_token}`,
          {
            headers: {
  ...
  async function fetch() {
    try {
      const res = await axios.get(
        `https://www.googleapis.com/analytics/v3/data/realtime?ids=ga:${gaID}&metrics=rt:activeUsers&access_token=${reponseGlobal.access_token}`,
        {
          headers: {
